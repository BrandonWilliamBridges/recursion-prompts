...
var arraySum = function(array) {
	...
	if(array.length > 1)
		return n + arraySum(array.slice(1));
	return n;
}
...
var palindrome = function(string) {
	...
	if(string[first] === string[last])
		return palindrome(string.substring(first + 1, last));
	return false;
};
...
var fizzBuzz = function(n) {
	if(n === 0)
		return [];
	return fizzBuzz(n - 1).concat([(n%3===0?'Fizz':'') + (n%5===0?'Buzz':'') + (((n%3!==0)&&(n%5!==0))?n.toString():'')]);
};
...
var countKeysInObj = function(obj, key) {
	...
	for(var myKey in obj) {
		total += ((key === myKey)?1:0) + ((typeof obj[myKey] === 'object') ? countKeysInObj(obj[myKey], key) : 0);
	}
	return total;
};
...
var countValuesInObj = function(obj, value) {
	var total = 0;
	for(var key in obj) {
		total += ((obj[key] === value) ? 1 : 0) + ((typeof obj[key] === 'object') ? countValuesInObj(obj[key], value) : 0);
	}
	return total;
};
...
var fibonacci = function(n) {
	...
	if(n === 1)
		return [0, 1];
	return fibonacci(n - 1).concat([fibonacci(n - 1)[n - 1] + fibonacci(n - 1)[n - 2]]);
};
...
var nestedEvenSum = function(obj) {
    var total = 0;
    for(var key in obj) {
    	total += (!isNaN(obj[key]) && (obj[key] % 2 === 0) ? obj[key] : 0) + (typeof obj[key] === 'object' ? nestedEvenSum(obj[key]) : 0);
    }
    return total;
};
...
var compress = function(list) {
    for(var x = 0; x < list.length - 1; x++) {
    	if(list[x] === list[x + 1])
    		return compress(list.slice(0, x + 1).concat(list.slice(x + 2)));
    }
    return list;
...
var alternateSign = function(array) {
		...
		return alternateSign(array.slice(0, array.length - 1)).concat(Math.abs(array[array.length - 1]) * -1);
};
...
var numToText = function(str) {
	...
	if(insert !== '')
		return insert + numToText(str.slice(1));
